...
RUN echo "deb https://packages.sury.org/php/ $(lsb_release -sc) main" | sudo tee /etc/apt/sources.list.d/php7.4.list

RUN apt-get -y update

RUN apt -y install apt-utils

RUN apt policy php7.4-cli

RUN apt policy php7.4-dev
